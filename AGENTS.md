...
**모든 작업자(사람 및 AI 에이전트)는 작업 완료 후 반드시 `works.log`에 기록을 추가해야 한다.**

### 형식
```
[YYYY-MM-DD] @github_id | 작업 요약 (1~2줄)
  - 변경 파일: 파일1, 파일2, ...
  - 세부 내용: 무엇을 왜 했는지
```

### 규칙
- `@github_id`: GitHub 아이디 또는 팀 내 식별 이름 기재
  - AI 에이전트는 사용 중인 도구명 기재 (예: `@claude`, `@cursor`, `@gemini`)
  - 사람과 AI가 함께 작업한 경우: `@사람id + @ai도구명` 형식
- 날짜: `YYYY-MM-DD` 형식, 한국 시간(KST) 기준
- 변경 파일: 실제로 생성·수정·삭제한 파일만 나열
- 세부 내용: 구조적 변경이나 설계 결정이 있는 경우 필수, 단순 수정은 생략 가능
...
6. **works.log 기록**: 작업 완료 후 `works.log`에 항목을 추가했는가?
